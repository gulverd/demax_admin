...
function draw(end){
    ...
    if(perimeter.length == 0){
        ...
        var obj = JSON.parse(JSON.stringify(perimeter));
        var res = [];
        for (var x  in obj)
            if (obj.hasOwnProperty(x))
                res.push(obj[x]['x']+','+obj[x]['y']);
...
function point_it(event) {
    ...
    if(event.ctrlKey || event.which === 3 || event.button === 2){
        if(perimeter.length==2){
            alert('მონახაზის შესაქმნელად საჭიროა მინიმუმ სამი წერტილი.');
            return false;
        }
        x = perimeter[0]['x'];
        y = perimeter[0]['y'];
        if(check_intersect(x,y)){
            alert('დაფიქსირდა ხაზების გადაკვეთა.');
            return false;
        }
        draw(true);
        alert('მონახაზი შექმნილია');
